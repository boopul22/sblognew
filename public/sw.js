// Optimized Service Worker for Critical Path Performance
// Reduces request chains and improves LCP through intelligent caching

const CACHE_VERSION = 'v2'
const CRITICAL_CACHE = `sayari-critical-${CACHE_VERSION}`
const STATIC_CACHE = `sayari-static-${CACHE_VERSION}`

// Critical resources that should be cached aggressively for instant loading
...
self.addEventListener('install', event => {
  console.log('🚀 Service Worker: Installing with critical path optimization')

  event.waitUntil(
    caches.open(CRITICAL_CACHE).then(cache => {
      console.log('📦 Caching critical resources for instant loading')
...
async function handleRequest(request) {
  ...
  try {
    // Strategy 1: Critical resources - Cache First (fastest LCP)
...
function isCriticalResource(pathname) {
  return pathname === '/' ||
         ...
         pathname.includes('react-vendor-')
}
...
console.log('🚀 Service Worker loaded with critical path optimization')
